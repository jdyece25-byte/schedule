...
| PC 반영 처리 | 승인 요청을 약 5초마다 별도로 확인합니다. 문서 다운로드 중에도 확인 처리를 진행하며, 검증 가능한 과제 마감과 선택한 변경의 검증·DB 저장은 PC에서 처리합니다. |
...
도입 당시 검증된 일정 392건은 초기 수집에서 변경하지 않고, 기존 자료를 관찰 기준으로 등록합니다. 이후 DB의 실제 일정 수는 늘거나 줄 수 있습니다. eTL에서 가져온 일정 ID·중복 방지 기록·수동 수정은 보존합니다. 주차만 적힌 자료에서 날짜를 만들거나 공휴일이라는 이유만으로 반복 수업을 삭제하지 않습니다. 암호화·손상 문서, 이미지·텍스트가 없는 PDF처럼 내용을 읽을 수 없는 자료는 확인 대상 정보로 남기며, 일정이 추출됐다고 가정하지 않습니다.
...
버튼을 누르면 즉시 전송 상태를 표시하고, 저장이 확인되면 해당 공지를 접힌 **처리 대기**로 옮깁니다. 비공개 저장소의 변경 불가 요청과 결과가 기준이므로 새로고침해도 접수 내역이 유지됩니다. 네트워크 실패는 완료로 숨기지 않습니다. 일부 후보만 반영하면 나머지 후보가 다시 나오고, 원문 변경·대상 충돌·실패도 확인 목록으로 돌아옵니다.

같은 원문 버전의 접수된 학교 공지는 반복 푸시와 달력 검토 배지에서 제외합니다. 이미 저장된 일정의 마감·이동·아침 요약 알림과 새 버전 공지는 유지합니다. 확인 요청 push는 문서를 다시 다운로드하지 않는 짧은 Actions 작업으로 분리했습니다. 클라우드에서도 읽음 처리는 완료할 수 있고, DB 반영 승인은 PC가 처리할 때까지 보존됩니다. 일시적 예약 경합은 다음 실행에서 재시도하며 인증·영구 오류는 계속 실패로 보고합니다.

수집기는 공지·과제·퀴즈 외에 강의계획서 본문, 페이지, 모듈, 접근 가능한 파일과 지원하는 외부 문서를 확인합니다. PDF는 한글과 표의 행을 보존하여 읽고 Office 문서도 추출합니다. 파일별 읽기 실패·분량 제한·지원하지 않는 형식은 수집 범위에 기록하므로 목록 API 연결 성공과 전체 자료 해석 성공을 구분합니다. 이미 읽은 문서는 캐시를 재사용합니다.

비공개 `school/index.json`에는 원문 버전별 학사 관련 지식 발췌가 저장됩니다. 자연어 일정 처리기는 관련 과목의 발췌·일정 후보·확인 상태를 함께 읽습니다. ‘확인했어요’로 닫아도 학사 규칙을 잊지 않습니다. 최신 사용자 변경을 우선하며, 요일이 바뀐 주차별 표는 기존 금요일 반복으로 뭉개지 않고 검토 후보로 올립니다. 날짜·시각이 불명확한 항목은 자동 확정하지 않습니다. 이는 모든 문서를 무제한으로 AI 해석하는 기능은 아니며, 읽지 못한 문서나 누락된 시간은 원문 확인이 필요합니다.

PC 수집은 API 공지·과제·퀴즈와 읽은 첨부파일 텍스트를 `%LOCALAPPDATA%/ScheduleSchool/knowledge`에 버전별로 보관합니다. 내려받은 첨부파일은 인접한 `document-cache`에 있습니다. 원문 ID·해시·행 번호와 함께 시험, 마감, 휴강·변경, 준비물 근거를 정리하며 이전 원문과 달라진 문구를 비교합니다. 분류는 검토를 돕는 근거이며 일정 확정이나 문서 전체의 의미를 이해했다는 보증이 아닙니다. 파일 형식·권한·크기 제한으로 못 읽은 자료는 실패 이력을 남기고 이전 성공 자료를 보존합니다. 일부 목록에서 자료가 빠져도 기존 일정이나 보관 자료를 자동 삭제하지 않습니다.

```powershell
# 다음 대화에서도 저장된 근거와 수집 상태 조회 (학교 API에 다시 접속하지 않음)
python -B src/school/archive.py --summary
python -B src/school/archive.py --query "시험" --course logic
python -B src/school/archive.py --category preparation --course leadership
```

학교 공지 화면은 **읽음 여부**와 **일정 미반영·일부 반영·반영 완료**를 따로 표시합니다. 읽음 처리한 후보는 보관 목록에서 다시 반영 요청할 수 있습니다. 최신 원문 버전이 바뀌면 이전 읽음 표시를 새 공지에 적용하지 않습니다. 수집기의 최근 시도, 마지막 자료 확보(부분 포함), 마지막 전체 성공, 읽기 실패·누락 건수도 구분합니다. 기존 기록에 부분 수집만 있으면 전체 성공 시각을 만들어 넣지 않습니다. 로컬 보관 실패는 표시하고 재시도하며, 이미 접수한 일정 처리 자체를 막지 않습니다.

사용자가 승인한 일정은 확인 표시를 남겨 다음 API 변경이 자동으로 덮어쓰지 못하게 합니다. 사용자가 지운 기존 회차도 새 원문이라는 이유만으로 재생성하지 않습니다. 새 원문과 충돌하는 이전 승인 요청은 반영 전에 다시 확인합니다. 파서 개선으로 동일 원문을 더 자세히 읽은 경우에는 이미 접수한 선택을 보존합니다.

각 자료의 **읽은 학사 규칙**을 펼치면 자연어 처리기에 전달되는 발췌를 확인할 수 있습니다. 다운로드 실패나 일시적 한도로 이미 읽은 지식을 지우지 않습니다. 실제 원문이 변경됐는데 읽지 못했으면 이전 자료임을 경고하고 새 확인 대상으로 남깁니다. 기존 학기 참고자료 중 일정 후보가 없는 파일은 접힌 지난 자료로 보관하며, 이후 새 공지는 다시 확인 목록에 표시합니다.
# 자연어 요청 사용량 절약

`오늘 일정`, `내일 일정 알려줘`, `9월 22일 일정 브리핑해줘`, `이번 주 일정`, `다음 주 일정`, `이번 달 일정`처럼 명확한 단독 조회는 최신 DB를 읽어 로컬에서 답하며 모델을 호출하지 않습니다. 사용자 요청일의 한국 날짜를 기준으로 계산합니다. 학교 공지 확인·수정 요청, 확인 질문에 대한 후속 답변, 긴 결과는 기존 모델 경로를 사용합니다. 로컬 조회는 미반영 학교 공지를 검토했다고 주장하지 않습니다.

모델 요청은 반복 문자열 사전과 표 형식으로 압축합니다. 전체 일정, 원본 인덱스, 취소 예외, 확인 필요 상태, 근거 문구는 유지합니다. 학교 자료의 암호학적 식별값만 일관된 짧은 별칭으로 전달하며 원본은 비공개 자료에 유지합니다. 실제 변경 검증은 압축되지 않은 최신 DB에서 수행합니다. 글자 수 감소율은 청구 토큰 감소율과 동일하지 않습니다.
