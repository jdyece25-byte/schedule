...
- 학교 공지 수집기 `%LOCALAPPDATA%/ScheduleSchool`도 작업 폴더와 별도로 실행됩니다. 일반 DB·화면 수정 때문에 이 프로그램을 중지·재설치하거나 `disabled` 파일을 만들지 않습니다. 기존 자연어 처리기·푸시 발송기도 계속 실행합니다. 수집은 약 15분마다, 승인 요청 확인은 약 30초마다이며, 로그인 중 매분 복구 작업이 상태를 확인합니다.
